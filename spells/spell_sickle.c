    ...
    for (int i = 0; i < 40; i++) {
        if (shoot.particles[i].init) {
            ...
            * shoot.particles[i].scale.y;
            ...
            update_sickle_particles2(g, shoot, i);
...
void sickle_particles(global_t *g, gameobject_t shoot)
{
    ...
    update_sickle_particles(g, shoot);
}
...
void sickle_update(void *tmp, int n)
{
    ...
    for (int i = 0; i < 40; i++) {
        ...
        if (g->spell[n].shoot[i].init) {
            ...
            sickle_particles(g, g->spell[n].shoot[i]);
            sickle_dammage(g, n, i);
        }
    }
}
